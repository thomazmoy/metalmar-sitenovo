...
<div class="card">
  ...
  <div class="card-header p-5 sm:p-6 border-b border-ld dark:border-darkborder flex items-center justify-between gap-4">
    <div class="flex items-center gap-4.5 shrink-0">
      ...
      <div>
        <h5 class="card-title text-xl font-bold text-dark dark:text-white leading-tight">Soluções & Serviços</h5>
        <p class="text-xs text-bodytext dark:text-darklink mt-1">Gerencie o portfólio de soluções e serviços prestados</p>
      </div>
    </div>
    <div class="flex items-center gap-3 justify-end shrink-0">
      <form action="/pesquisa" method="get" class="relative flex items-center w-60 sm:w-72">
        <input type="text" name="pesquisa" class="form-control w-full h-10 pl-3.5 pr-10 rounded-xl text-sm" placeholder="Pesquisar soluções..." value="{{ Request('pesquisa') }}">
        ...
        <tr class="hover:bg-slate-50/70 dark:hover:bg-darkgray/40 transition-colors">
          ...
          <td class="px-6 py-4.5">
            <label class="relative inline-flex items-center cursor-pointer">
              <input type="checkbox" class="sr-only peer toggle-situacao-index" data-model="Solucao" data-id="{{ $solucao->id }}" {{ $solucao->situacao == 1 ? 'checked' : '' }}>
          ...
          <td class="px-6 py-4.5">
            <div class="flex items-center justify-center gap-2">
              <a href="{{ route('solucao.edit', $solucao->id) }}" class="btn-action-edit" title="Editar">
                <i class="ti ti-pencil"></i>
              </a>
              <button type="button" onclick="openDeleteModal('{{ $solucao->id }}')" class="btn-action-delete" title="Excluir">
          ...
          <td colspan="4" class="px-6 py-12 text-center">
            <div class="flex flex-col items-center justify-center">
              <div class="w-16 h-16 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-400 dark:text-slate-500 mb-3">
                <i class="ti ti-tools-off text-3xl"></i>
              </div>
              <p class="text-base font-medium text-dark dark:text-white">Nenhuma solução encontrada</p>
              <p class="text-xs text-bodytext dark:text-darklink mt-1">Clique no botão acima para cadastrar uma nova solução.</p>
            </div>
          </td>
        </tr>
        @endforelse
  ...
  @if($solucoes->hasPages())
  <div class="px-6 py-4 border-t border-ld dark:border-darkborder">
    {{ $solucoes->appends(Request::get('pesquisa'))->links() }}
  </div>
  @endif
</div>
