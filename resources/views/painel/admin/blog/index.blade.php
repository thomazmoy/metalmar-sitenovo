...
<div class="card">
  ...
  <div class="card-header p-5 sm:p-6 border-b border-ld dark:border-darkborder flex items-center justify-between gap-4">
    <div class="flex items-center gap-4.5 shrink-0">
      ...
      <div>
        <h5 class="card-title text-xl font-bold text-dark dark:text-white leading-tight">Blog & Notícias</h5>
        <p class="text-xs text-bodytext dark:text-darklink mt-1">Gerencie as publicações e artigos informativos do site</p>
      </div>
    </div>
    <div class="flex items-center gap-3 justify-end shrink-0">
      <form action="/procurar" method="get" class="relative flex items-center w-60 sm:w-72">
        <input type="text" name="procurar" class="form-control w-full h-10 pl-3.5 pr-10 rounded-xl text-sm" placeholder="Pesquisar artigos..." value="{{ Request('procurar') }}">
    ...
    <table class="min-w-full whitespace-nowrap text-sm">
      ...
      <tbody class="divide-y divide-ld dark:divide-darkborder">
        ...
        <tr class="hover:bg-slate-50/70 dark:hover:bg-darkgray/40 transition-colors">
          ...
          <td class="px-6 py-4.5 font-semibold text-dark dark:text-white max-w-xs truncate">
            {{ $blog->titulo }}
          </td>
          ...
          <td class="px-6 py-4.5">
            <label class="relative inline-flex items-center cursor-pointer">
              <input type="checkbox" class="sr-only peer toggle-situacao-index" data-model="Blog" data-id="{{ $blog->id }}" {{ $blog->situacao == 1 ? 'checked' : '' }}>
          ...
          <td class="px-6 py-4.5">
            <div class="flex items-center justify-center gap-2">
              <a href="{{ route('blog.edit', $blog->id) }}" class="btn-action-edit" title="Editar">
                <i class="ti ti-pencil"></i>
              </a>
              <button type="button" onclick="openDeleteModal('{{ $blog->id }}')" class="btn-action-delete" title="Excluir">
          ...
          <td colspan="6" class="px-6 py-12 text-center">
            <div class="flex flex-col items-center justify-center">
              <div class="w-16 h-16 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-400 dark:text-slate-500 mb-3">
                <i class="ti ti-news-off text-3xl"></i>
              </div>
              <p class="text-base font-medium text-dark dark:text-white">Nenhum artigo encontrado</p>
              <p class="text-xs text-bodytext dark:text-darklink mt-1">Clique no botão acima para adicionar uma nova publicação.</p>
            </div>
          </td>
        </tr>
        @endforelse
