...
<div class="card">
  ...
  <div class="card-header p-5 sm:p-6 border-b border-ld dark:border-darkborder flex items-center justify-between gap-4">
    <div class="flex items-center gap-4.5 shrink-0">
      ...
      <div>
        <h5 class="card-title text-xl font-bold text-dark dark:text-white leading-tight">Galeria de Fotos</h5>
        <p class="text-xs text-bodytext dark:text-darklink mt-1">Gerencie as imagens e acervo visual da empresa</p>
      </div>
    </div>
    <div class="flex items-center gap-3 justify-end shrink-0">
      <a href="{{ route('galeria.create') }}" class="h-10 px-4 rounded-xl btn btn-primary flex items-center gap-2 text-sm font-semibold shadow-sm hover:shadow-md transition-all">
      ...
      <tbody class="divide-y divide-ld dark:divide-darkborder">
        ...
        <tr class="hover:bg-slate-50/70 dark:hover:bg-darkgray/40 transition-colors">
          <td class="px-6 py-4.5">
            @if($galeria->img ?? '')
              <img src="{{ url('storage/'.$galeria->img) }}" class="w-16 h-12 rounded-xl object-cover border border-ld dark:border-darkborder shadow-xs">
            @else
          ...
          <td class="px-6 py-4.5 font-semibold text-dark dark:text-white">
            {{ $galeria->titulo ?? 'Foto #'.$galeria->id }}
          </td>
          <td class="px-6 py-4.5">
            <label class="relative inline-flex items-center cursor-pointer">
              <input type="checkbox" class="sr-only peer toggle-situacao-index" data-model="Galeria" data-id="{{ $galeria->id }}" {{ $galeria->situacao == 1 ? 'checked' : '' }}>
          ...
          <td class="px-6 py-4.5">
            <div class="flex items-center justify-center gap-2">
              <a href="{{ route('galeria.edit', $galeria->id) }}" class="btn-action-edit" title="Editar">
                <i class="ti ti-pencil"></i>
              </a>
              <button type="button" onclick="openDeleteModal('{{ $galeria->id }}')" class="btn-action-delete" title="Excluir">
        ...
        <tr>
          <td colspan="4" class="px-6 py-12 text-center">
            <div class="flex flex-col items-center justify-center">
              ...
              <p class="text-base font-medium text-dark dark:text-white">Nenhuma foto cadastrada</p>
              <p class="text-xs text-bodytext dark:text-darklink mt-1">Clique no botão acima para adicionar fotos à galeria.</p>
            </div>
          </td>
        </tr>
        @endforelse
...
<div id="deleteModal" class="ms-modal hidden">
  ...
  <div class="ms-modal-content max-w-md mx-auto p-6">
    ...
    <p class="text-center text-sm text-bodytext dark:text-darklink mb-6">Tem certeza de que deseja remover esta foto? Esta ação não poderá ser desfeita.</p>
    <form action="{{ route('galeria.destroy','galeria') }}" method="post" class="flex gap-3 justify-center">
      {{ method_field('delete') }}{{ csrf_field() }}
      <input type="hidden" name="galeria_id" id="delete_id" value="">
