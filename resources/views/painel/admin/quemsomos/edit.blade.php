...
<div class="card">
  ...
  <div class="card-header p-5 sm:p-6 border-b border-ld dark:border-darkborder flex items-center justify-between gap-4">
    <div class="flex items-center gap-4.5 shrink-0">
      ...
      <div>
        <h5 class="card-title text-xl font-bold text-dark dark:text-white leading-tight">Quem Somos</h5>
        <p class="text-xs text-bodytext dark:text-darklink mt-1">Gerencie a história institucional, fotos, vídeos e traduções (EN / ES)</p>
      </div>
    </div>
  </div>
  <div class="card-body">
    <form action="{{ route('quemsomos.update', $quemsomos) }}" method="POST" enctype="multipart/form-data">
      @method('PUT')
