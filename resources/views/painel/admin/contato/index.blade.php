...
<title>MS System | Contatos</title>
@section('content')
...
<div class="card">
  ...
  <div class="card-header p-5 sm:p-6 border-b border-ld dark:border-darkborder flex items-center justify-between gap-4">
    <div class="flex items-center gap-4.5 shrink-0">
      ...
      <div>
        <h5 class="card-title text-xl font-bold text-dark dark:text-white leading-tight">Contatos Recebidos</h5>
        <p class="text-xs text-bodytext dark:text-darklink mt-1">Gerencie e visualize as mensagens enviadas pelos visitantes do site</p>
      </div>
    </div>
    ...
    <div class="flex items-center gap-3 justify-end shrink-0">
      <form action="{{ route('search') }}" method="get" class="relative flex items-center w-60 sm:w-72">
        <input type="text" name="search" class="form-control w-full h-10 pl-3.5 pr-10 rounded-xl text-sm" placeholder="Pesquisar mensagens..." value="{{ Request('search') }}">
      ...
      <a href="{{ route('exportar') }}" class="h-10 px-4 rounded-xl btn btn-outline-secondary flex items-center gap-2 text-xs font-semibold shadow-xs hover:border-emerald-500 hover:text-emerald-600 dark:hover:text-emerald-400 transition-all" title="Exportar para Excel">
        <i class="ti ti-file-spreadsheet text-emerald-600 dark:text-emerald-400 text-base"></i>
          ...
          <td colspan="5" class="px-4 py-12 text-center">
            <div class="flex flex-col items-center justify-center">
              <div class="w-16 h-16 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-400 dark:text-slate-500 mb-3">
                <i class="ti ti-mail-off text-3xl"></i>
              </div>
              <p class="text-base font-medium text-dark dark:text-white">Nenhum contato encontrado</p>
              <p class="text-xs text-bodytext dark:text-darklink mt-1">Ainda não há mensagens recebidas no sistema.</p>
            </div>
          </td>
        </tr>
        @endforelse
